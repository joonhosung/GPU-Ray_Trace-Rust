...
* Biplane: A complex scene depicting a plane over snowy terrain, composed of 7,316 and 267,958 triangles each
...
| biplane | 100 | 5 | 2 | DNF (~1025s/it) | DNF |
| biplane | 1000 | 1 | 2 | DNF (~1045s/it) | - |
| biplane | 1000 | 2 | 8 | 23499.1s | - |
| biplane | 10000 | 1 | 2 | DNF (~885s/it) | - |
...
3. Pre-Rasterization Enhancement
   ...
   * analyze image quality with/without pre-rasterization with the same number of iterations
...
2. Animation System Integration
     * Smooth object movement interpolation using the Keyframe crate.    
     * Parallelized frame generation saved as a video using the opencv crate's videoio::videowriter module
...
* Compute Shader Development (Both)
  ...
  * Continuous performance benchmarking while optimizations are applied
  * Jackson will take on more work here since Jun Ho will work on keyframe integration

* Keyframe Integration (Jun Ho)
  * Begin implementation of frame interpolation
  * At this point, the animations won't be rendered, but the object locations for each frame should be calculated
...
* Animation Development (Pending speedup goals)
  ...
  * Implement multi-frame generation (Jun Ho)
...
* Project Documentation (Both)
  ...
  * analyze and summarize all optimizations made and why they were effective/ineffective
